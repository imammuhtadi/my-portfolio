...
export async function GET(request: NextRequest) {
  ...
  try {
    const filePath = path.join(process.cwd(), 'public', 'tracegrid', 'app-release.apk')
    ...
    const response = new NextResponse(fileBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="tracegrid-app-release.apk"',
    ...
    const speedMBps = (parseFloat(fileSizeInMB) / parseFloat(duration)).toFixed(2)
